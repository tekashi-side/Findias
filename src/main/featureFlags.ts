...
const FLAG_RESOLVERS: Record<FeatureFlag, () => boolean> = {
  /** Dev-only: including prerelease Uiscias builds in the catalog is only offered
   * when the app is unpackaged. In a packaged build the flag is off, which hides
   * the UI toggle and force-disables the effective value regardless of what
   * `findias-settings.json` contains. */
